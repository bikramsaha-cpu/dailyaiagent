...
def search_product(page):
    ...
    search_box = 'xpath=//*[@id="search_string"]'
    page.click(search_box)

    # Step 2: Type with delay to avoid auto-clear
    page.type(search_box, "wooden bed", delay=80)

    # Step 3: Confirm value (retry once if needed)
    typed_value = page.input_value(search_box)
    if typed_value.strip() == "":
        # Retry typing if IndiaMART cleared it
        page.type(search_box, "wooden bed", delay=80)
        page.wait_for_timeout(300)

    # Step 4: Small wait â†’ ensures no overwrite/clear
    page.wait_for_timeout(500)

    # Step 5: Click Advanced Search button
    page.click('xpath=//*[@id="root"]/div[1]/div[2]/div[1]/div/div[1]/div[2]/button')
    

    # Step 6: Click on Search inside advanced
    page.wait_for_selector("button.adv-search-button", timeout=20000)

    with page.context.expect_page() as new_page_info:
        page.click("button.adv-search-button")

    new_page = new_page_info.value
    new_page.wait_for_load_state("domcontentloaded")

    # Step 7: Close results and return to dashboard
    time.sleep(2)
    new_page.close()
    page.bring_to_front()
