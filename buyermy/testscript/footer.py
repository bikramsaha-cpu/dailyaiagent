...
def test_mobile_apps_links(page, log_step):
    for desc, xpath, new_tab in MOBILE_APPS_LINKS:
        log_step(f"Verify Navigation of {desc}", lambda: click_footer_link(page, desc, xpath, new_tab))

def test_social_links(page, log_step):
    for desc, xpath, new_tab in SOCIAL_LINKS:
        log_step(f"Verify Navigation of {desc}", lambda: click_footer_link(page, desc, xpath, new_tab))

def test_terms_and_policies_links(page, log_step):
    for desc, xpath in TERMS_AND_POLICIES_LINKS:
        log_step(f"Verify Navigation of {desc}", lambda: click_footer_link(page, desc, xpath))
...
def run(playwright):
    ...
    for browser_name in BROWSERS:
        ...
        test_mobile_apps_links(page, log_step)
        test_social_links(page, log_step)
        test_terms_and_policies_links(page, log_step)
