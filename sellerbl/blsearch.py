...
def main():
    ...
    # Step 0: Initialize Google Sheet Logger
    try:
    ...
    with sync_playwright() as playwright:
        for browser_name in BROWSERS:
            ...
            try:
                ...
                # Step 2: Click on BuyLeads tab
                try:
                    page.wait_for_selector('a:has(h5:has-text("Relevant BuyLeads"))', timeout=10000)
                    page.click('a:has(h5:has-text("Relevant BuyLeads"))')
                    log_step("Navigate to BuyLeads", "Pass", "Navigation successful")
                except Exception as e:
                    log_step("Navigate to BuyLeads", "Fail", str(e))
                    continue

                # Step 3: Search a keyword for BuyLead Search
                try:
                    page.wait_for_selector("#search_string", timeout=5000)
                    page.click("#search_string", force=True)
                    page.type("#search_string", "Basmati Rice", delay=200)
                    if page.input_value("#search_string") != "Basmati Rice":
                        raise Exception("Full text not entered!")
                    page.click("#btnSearch", force=True)
                    print("âœ… Search executed for: Basmati Rice")
                except Exception as e:
                    log_step("Search BuyLeads for 'Basmati Rice'", "Fail", str(e))
                    continue

                 # Step 3: Apply Filters
                # Click on "More Filters"
                more_filters_btn = page.locator("#vw_svd_fltr_btn_id")
                more_filters_btn.click()
                page.wait_for_timeout(5000)  # wait for panel to open

                # Check if "No saved filter" is displayed
                no_filter_locator = page.locator("#no_saved_filter")
                if no_filter_locator.is_visible():
                    print("â„¹ No saved filters found. Closing popup.")
                    cross_btn = page.locator("span[onclick*='closed_View_saved_fltr();']")
                    cross_btn.click()
                else:
                    # Count saved filters
                    all_filters = page.locator("#indexed_DB_saved_filter_all li")
                    visible_filters = all_filters.evaluate_all(
                        "els => els.filter(el => getComputedStyle(el).display !== 'none')"
                    )
                    filter_count = len(visible_filters)

                    # saved_filters = page.locator("#indexed_DB_saved_filter_all li")
                    ...
                    if filter_count >= 5:
                        ...
                        while saved_filters.count() >= 5:
                            ...
                            edit_icon = first_filter.locator("img.recommtltp")
                            edit_icon.wait_for(state="visible", timeout=10000)
                            edit_icon.click()
                ...
                # Step 10: Hide BuyLead
                try:
                    page.wait_for_selector("#hidebl1", timeout=5000)
                    page.click("#hidebl1")
                    log_step("Hide BuyLead", "Pass", "BL hidden successfully")
                except Exception as e:
                    log_step("Hide BuyLead", "Fail", str(e))
                    continue

                # Step 11: Unhide BuyLead
                try:
                    page.wait_for_selector("#unhide_cstm_content_1", timeout=5000)
                    page.click("#unhide_cstm_content_1")
                    page.wait_for_timeout(3000)
                    log_step("Unhide BuyLead", "Pass", "Clicked 'Unhide' button successfully")
                except Exception as e:
                    log_step("Unhide BuyLead", "Fail", str(e))
                    continue
                ...
                try:
                        ...
                        page.wait_for_timeout(2000)
                            ...
                            print(f"âš ï¸ Failed to navigate back after Buy Now {i + 1}: {e}")

                # Step 17: Close browser
                try:
                    browser.close()
                except Exception as e:
                    print("âš ï¸ Failed to close browser:", e)

            except Exception as e:
                print(f"âŒ Error in browser {browser_name}: {e}")
