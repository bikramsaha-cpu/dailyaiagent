...
class GoogleSheetLogger:
    ...
    def get_failed_cases(self, start_time=None, end_time=None, tab_name=None):
        """
        Fetch all failed test cases with their details
        (Test Title, Remarks, Browser, Phone, Date, Time).
        """
        sheet = self.sheet if tab_name is None else self.sheet.spreadsheet.worksheet(tab_name)
        all_rows = sheet.get_all_values()[1:]  # Skip header
        ...
        for row in all_rows:
            try:
                if len(row) > 1 and row[1].strip().lower() == "fail":
                    failed_cases.append({
                        "test_title": row[0] if len(row) > 0 else "",
                        "status": row[1],
                        "remarks": row[2] if len(row) > 2 else "",
                        "browser": row[3] if len(row) > 3 else "Unknown",
                        "phone": row[4] if len(row) > 4 else "Unknown",
