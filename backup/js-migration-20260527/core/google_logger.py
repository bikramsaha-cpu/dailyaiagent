...
class GoogleSheetLogger:
    ...
    _header_cache = {}
    _records_cache = {}
    _cache_ttl_seconds = int(os.getenv("AUTOMATION_SHEET_CACHE_SECONDS", "45"))

    def __init__(self, sheet_name: str, tab_name: str):
        client = self._client()
        spreadsheet = self._spreadsheet(client, sheet_name)
        self.sheet = self._worksheet(spreadsheet, sheet_name, tab_name)
    ...
    def _client(cls):
        if cls._client_cache is None:
            ...
            creds = cls._load_credentials(scope)
            cls._client_cache = gspread.authorize(creds)
        return cls._client_cache

    @classmethod
    def _spreadsheet(cls, client, sheet_name: str):
        if sheet_name not in cls._spreadsheet_cache:
            cls._spreadsheet_cache[sheet_name] = client.open(sheet_name)
        return cls._spreadsheet_cache[sheet_name]

    @classmethod
    def _worksheet(cls, spreadsheet, sheet_name: str, tab_name: str):
        cache_key = (sheet_name, tab_name)
        if cache_key not in cls._worksheet_cache:
            cls._worksheet_cache[cache_key] = spreadsheet.worksheet(tab_name)
        return cls._worksheet_cache[cache_key]

    @classmethod
    def _sheet_cache_key(cls, sheet) -> tuple[str, str]:
        spreadsheet_title = getattr(sheet.spreadsheet, "title", "")
        return (spreadsheet_title, sheet.title)

    @classmethod
    def _headers(cls, sheet) -> list[str]:
        cache_key = cls._sheet_cache_key(sheet)
        if cache_key not in cls._header_cache:
            cls._header_cache[cache_key] = sheet.row_values(1)
        return cls._header_cache[cache_key]

    @classmethod
    def _ensure_test_step_header(cls, sheet) -> list[str]:
        headers = cls._headers(sheet)
        if "Test Step" in headers:
            return headers
        if headers[:2] == ["Test Title", "Status"]:
            sheet.insert_cols([["Test Step"]], col=2)
            cache_key = cls._sheet_cache_key(sheet)
            cls._header_cache.pop(cache_key, None)
            cls._records_cache.pop(cache_key, None)
            return cls._headers(sheet)
        return headers

    @classmethod
    def _invalidate_records(cls, sheet) -> None:
        cache_key = cls._sheet_cache_key(sheet)
        cls._records_cache.pop(cache_key, None)
    ...
    def log_status(
        ...
        headers = self._ensure_test_step_header(sheet)
        values_by_header = {
            "Test Title": test_title,
            "Test Step": test_step or "",
            "Status": status,
            "Remarks": remarks,
            "Browser": browser,
            "Phone": phone,
        ...
        if "Test Step" in headers:
            sheet.append_row([values_by_header.get(header, "") for header in headers])
        else:
    ...
    def read_rows(self, tab_name: str | None = None, include_header: bool = False) -> list[list[str]]:
        sheet = self.sheet if tab_name is None else self.sheet.spreadsheet.worksheet(tab_name)
        cache_key = self._sheet_cache_key(sheet)
        cached = self._records_cache.get(cache_key)
        now = time.monotonic()
        if cached and now - cached["created_at"] <= self._cache_ttl_seconds:
            rows = cached["rows"]
        else:
            rows = sheet.get_all_values()
            self._records_cache[cache_key] = {"created_at": now, "rows": rows}
        return rows if include_header else rows[1:]

    def refresh_records(self, tab_name: str | None = None) -> None:
        sheet = self.sheet if tab_name is None else self.sheet.spreadsheet.worksheet(tab_name)
        self._invalidate_records(sheet)
