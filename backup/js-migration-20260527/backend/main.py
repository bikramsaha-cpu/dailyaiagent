...
from core.seo_services import get_pagespeed_insights, get_search_console_data
...
def workspace_settings_env(request: WorkspaceSettingsRequest) -> dict[str, str]:
    candidates = {
        ...
        "AUTOMATION_SMTP_RECIPIENTS": request.smtp_recipients,
        "AUTOMATION_GOOGLE_API_KEY": request.google_api_key,
    }
    values = {
        key: value.strip()
        for key, value in candidates.items()
...
@app.post("/api/seo/pagespeed")
def pagespeed(request: PageSpeedRequest) -> dict[str, Any]:
    try:
        return get_pagespeed_insights(
            request.url,
            strategy=request.strategy,
            google_api_key=request.google_api_key,
        )
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Could not load PageSpeed Insights: {exc}") from exc


@app.post("/api/seo/search-console")
def search_console(request: SearchConsoleRequest) -> dict[str, Any]:
    try:
        return get_search_console_data(
            request.url,
            site_url=request.site_url,
            days=request.days,
        )
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Could not load Search Console data: {exc}") from exc
