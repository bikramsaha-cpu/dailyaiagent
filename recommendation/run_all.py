...
scripts_to_run = [
    ("buyer_login.py", None),
    (os.path.join("testscript", "pdp", "test_pdp.py"), "Recommendation"),
    (os.path.join("testscript", "dir", "test_impcat.py"), "Recommendation"),
    (os.path.join("testscript", "search", "test_search.py"), "Recommendation"),
    (os.path.join("testscript", "companyPage", "test_company_page.py"), "Recommendation"),
]
...
for script_rel_path, _ in scripts_to_run:
    ...
    if result.returncode != 0:
        print(f"{script_rel_path} exited with code {result.returncode}")
        if script_rel_path == "buyer_login.py":
            login_failed = True
            print("Login step failed. Aborting remaining Recommendation scripts.")
            break

if login_failed:
    print("\nRecommendation run stopped because login/session setup failed.")
    sys.exit(1)
...
for tab_name in sorted({tab for _, tab in scripts_to_run if tab}):
    try:
        ...
        print(f"Could not fetch summary for {tab_name}: {exc}")

summary_payload = {
    "suite_name": "Recommendation",
    "module_name": "Desktop Recommendation",
...
if total_total > 0:
    try:
        send_summary_email(
            ...
            total=total_total,
            sheet_url=SHEET_URL,
            tab_name="Desktop Recommendation",
